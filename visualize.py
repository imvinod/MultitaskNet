...
class Visualize:
    def __init__(self, opt, model=None, test_loader=None):
        ...
        self.new_image = Image.new('RGB', (1124, 240))

        self.save_path = os.path.join(self.opt.results_dir, self.opt.name, 'visualization_1')
    ...
    def paint_and_save(self, image, depth_label, depth_pred_image, rgb_image, scene_label, scene_pred, idx):
            ...
            depth_label = Image.fromarray(depth_label, mode="L")
            x_offset += image.size[0]
            self.new_image.paste(depth_label, (x_offset, 0))

            depth_image = Image.fromarray(depth_pred_image, mode="L")
            #depth_image.save(os.path.join(self.save_path, 'depprediction_' + str(idx+1) + '.png'))
            x_offset += depth_label.size[0]
            self.new_image.paste(depth_image, (x_offset, 0))
    ...
    def visualize_predictions(self):
        ...
        if self.test_loader is None:
            _, test_data = get_data(self.opt, self.dset_info, use_train=False, use_test=True, visualize = True)
        ...
        if self.model is None:
            # Read the MultiTaskNet model path that will be used for prediction and load the weights to the initialized model
            self.model = MultiTaskNet(seg_class_num, self.opt.gpu_id, self.opt.use_class)

            checkpoint = torch.load(self.model_path)
            self.model.load_state_dict(checkpoint['state_dict'])
            print("[INFO] Weights from pretrained MultiTaskNet model has been loaded. Checkpoint: %s" % self.model_path)
        ...
        for num, batch in enumerate(self.test_loader):
            ...
            if self.opt.use_class:
                ...
                test_seg_outputs, output_depth = self.model(test_rgb_inputs)
            ...
            # deph image
            test_depth_inputs = test_depth_inputs.data.cpu().numpy()[0].transpose(1, 2, 0)
            test_depth_inputs = np.squeeze(test_depth_inputs, axis = 2)

            output_depth = output_depth.data.cpu().numpy()[0].transpose(1, 2, 0)            
            output_depth = np.squeeze(output_depth, axis = 2)
            # Color semantic segmentation labels, print scene classification labels, and save comparison images
            self.paint_and_save(comparison_images, np.uint8(test_depth_inputs), np.uint8(output_depth), np.uint8(test_rgb_inputs), test_class_labels, test_class_preds, num)
